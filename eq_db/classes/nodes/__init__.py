...
def make_nodes(tsid={}, tdate=''):
    ...
    # mapper(Node, Schemas.nodes)
    # Node.__table__.metadata.drop_all(engine)
    ...
    process_nodes(nodes)
    # print(ORM.session.query(Node).filter(Node.node_code==514986).one())
    ORM.session.commit()
...
# class Node(ORM.Base):
#     __tablename__ = 'nodes'
#     node_code = Column(Integer, primary_key=True)
#     area = Column(Integer)
...
#
#     # hours = relationship('NodeHourData', back_populates='node')
#     hours = relationship('NodeHourData', back_populates='node')
#
