...
def guess_header(array, name=''):
    ...
    def squeeze(array):
        ...
        if isinstance(array, (Sequence, set)) \
           and not isinstance(array, basestring) and len(array) == 1:
            (datum,) = array
            return squeeze(datum)
        else:
            return array
    ...
    if isinstance(array, ZeroSize):
        header.update({
            'mclass': 'mxDOUBLE_CLASS', 'mtp': 'miDOUBLE', 'dims': (0, array.col_num)
        })
        array = []
    ...
    #         'mclass': 'mxINT32_CLASS', 'mtp': 'miINT32', 'dims': (1, 1)})
        ...
        if isarray(array, lambda i: isinstance(i, numbers.Real), 1):
            # 1D double array
            header.update({
                ...
                or isarray(array, lambda i: isinstance(i, (set, Mapping)), 1) \
                or isarray(array, lambda i: isinstance(i, (set, Mapping))):
            ...
            elif isarray(array, lambda i: isinstance(i, numbers.Real)):
                # 2D double array
                header.update({
            ...
            elif isarray(array, lambda i: isinstance(i,
                                        (basestring, Sequence, Mapping))):
